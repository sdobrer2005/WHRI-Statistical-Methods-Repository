...
- **Shapiro–Wilk Test**  
  Assesses normality by evaluating the correlation between ordered sample values and the corresponding expected values under a normal distribution.  
  This test is particularly sensitive to general departures from normality and performs well in small to moderate sample sizes.
